...
*** DONE fix dangling popover on track delete
*** DONE new track => edit title => save => reset => edit title => save - changes the new track
*** DONE title changes don't propagate to other playlists
*** DONE metricals: jump around as pulse/BPM change
*** DONE playlist title input extends past right edge
*** DONE replace metronome toggle with play/stop
*** DONE fix mute on track delete
*** DONE fix MM popover div/ul/li thing
*** DONE large infinity char
*** DONE info: 1+1+47
*** DONE reset :hover style
*** DONE change toggle to start/stop
*** DONE reset UI if selected track is removed from playlist
*** DONE spacebar in editor starts/stops metronome

*** TODO can't edit new track
      - toolbar edit icon is not visible
      - also: track is added to end of playlist but BOF/EOF aren't updated
      - toolbar edit icon is visible after track deleted
...
**** TODO cascade update following sections after pick pulse
...
*** TODO Firefox
**** DONE set width/height for all SVGs
**** DONE time-signature layout
**** DONE pulse layout
**** DONE controls layout
**** DONE editor section layout
**** DONE time signature popover
**** DONE MM signature popover
**** DONE section MM signature popover
**** DONE editor time signature popover
**** TODO editor: loop count font
**** TODO remove up/down arrows in editor BPM
...
*** TODO rework UI
      - add '+' button to tracklist to create new track
      - '+' button in All Tracks creates new track in track editor
      - delete track in 'All Tracks' deletes track from all playlists and DB

*** TODO editor: toolbar play/stop
...
*** TODO consistently pass the whole track as a parameter (and not just the UUID)
...
** Sound sets
*** TODO store sounds in IndexedDB
*** TODO has
*** TODO get 
*** TODO versioning (newer versions get downloaded)
*** TODO soft/medium/loud sets
*** TODO (?) use microtasks to load
