...
*** DONE track loop
*** DONE move BOF/EOF to model/playlist
*** DONE URL playlist parameter
*** DONE fix flicker on list drag
*** DONE move to github.transcriptaze
*** DONE commit as user transcriptaze
*** DONE fix portrait + fullscreen layout
*** DONE remove settings from toolbar entirely

*** TODO loop count
**** DONE editor
**** TODO count-down in metronome

*** TODO add new track playlists:tracklist
      - playlist weirdness on open if new track is added to selected+closed playlist
      - something to do with the isopen/selected logic because the handles are showing
      - arbitrarily makes 'All Tracks' selected sometimes
...
*** TODO Firefox MM/time signature CSS
