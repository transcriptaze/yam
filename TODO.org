...
*** DONE refactor MM to use track for redraw
*** DONE refactor time signature to use datastore
*** DONE Fix MM/time signature subsections
*** DONE pads initialisation (doesn't display actual last pads)

*** TODO fix worklet tempo/BPM thing
      - not doing the 160 BPM thing for (debug)

*** TODO update worklet track on editor::save
...
**** DONE increase padding at bottom of section (clef tail is too close to edge)
**** DONE fix BPM defval - should inherit from previous section
**** TODO fix time signature defval - should inherit from previous section
**** TODO slide in on initialisation
**** TODO lowlight empty section fields (name, role)
      - placeholder ?
**** TODO disable/hide section time signature and tempo if subsections
      - rather just list subsections
