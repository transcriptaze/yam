...
**** DONE on selecting another playlist
**** DONE on load from file
**** DONE stop engine if playing
...
*** TODO playlists '+'' floating button is visually confusing
      - (maybe) hide unless everything is closed
      - (rather) blur list underlying button 
