...
export class Metronome extends AudioWorkletProcessor {
  ...
  process(_inputs, outputs, parameters) {
    ...
    const BPM = this.section?.tempo ?? clamp(parameters.BPM[0], 40, 200)
  ...
  flip(state, section, bar, beat, parameters) {
    ...
    if (section != null) {
      ...
      this.state.timeSignature = section?.timeSignature
      ...
      this.state.timeSignature = this.track?.timeSignature
      // this.state.pulse = clamp(parameters.pulse[0], 0.125, 0.75)
